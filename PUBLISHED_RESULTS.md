# Ramanujan primes a(1)-a(23): values and timing for publication

See `published_results.csv` for the machine-readable table. Methodology notes:

**a(23) is certified but its timing is NOT clean** -- see the dedicated
note after a(22) below before using it for anything.
...
- **a(23):** certified 2026-08-14 on the desktop machine (949,634,047,203,617,038,366),
  cross-algorithm verified, its offset-42 dip independently reproduced via
  Miller-Rabin, and its primary anchor pi(10^23) matches an independently
  published external reference value exactly -- the value itself is as
  solid as any other term here. **Its recorded time (95,911s / 26h38m31s)
  is NOT clean and must not be used for complexity comparisons.** That
  run resumed from checkpoints cached during earlier interrupted attempts
  and ran concurrently with an unrelated workload on the same machine, so
  the recorded time understates the true from-scratch cost -- the same
  contamination pattern a(21)/a(22) originally had, and unlike them,
  a(23) has not yet gotten the fresh cache-bypassed re-run that would fix
  it. See `TIME_COMPLEXITY_ANALYSIS.md` for what that re-run would take
  and why 95,911s is excluded from every fitted growth rate there.
...
a(23), which is why a(23) was computed on a separate, higher-RAM desktop
instead -- see the paper's Section 3.5 for that machine's specs).
...
bounds. a(20)-a(23) additionally cross-verify their record-defining anchors
