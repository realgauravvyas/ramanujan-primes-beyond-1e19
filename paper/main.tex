...
$k=20,21,22,23$, computed on consumer hardware with no institutional
resources. The method combines
...
This paper reports a certified extension of $a(k)$ to $k=20,21,22,23$.
...
Write $f(x) = \pcount(x) - \pcount(x/2)$. Note that $f$ is \emph{not}
monotone --- it increases by $1$ at each prime and decreases by $1$ at
each doubled prime, and its local dips below $f(Q)$ are precisely what
the certification must rule out (or locate exactly). What \emph{is}
monotone is $\pcount$ itself, and that alone yields the two elementary
facts on which the entire method rests.

\begin{proposition}[Counting identity]
\label{prop:identity}
For every integer $x \geq 2$, the number of Ramanujan primes $\leq x$
equals $\displaystyle\min_{y \geq x} f(y)$, and the minimum is attained.
\end{proposition}

\begin{proof}
Since $f$ is integer-valued and $f(y) \to \infty$, the minimum over
$y \geq x$ is attained. By definition $R_n$ is the least integer such
that $f(y) \geq n$ for all $y \geq R_n$. If $R_n \leq x$ then $f(y)
\geq n$ for all $y \geq x$; conversely, if $f(y) \geq n$ for all
$y \geq x$ then $R_n \leq x$ by minimality of $R_n$. Hence
$\{\,n : R_n \leq x\,\} = \{\,n : n \leq \min_{y \geq x} f(y)\,\}$,
which is an initial segment of cardinality $\min_{y \geq x} f(y)$.
\end{proof}
...
\begin{itemize}
  ...
  \]
  Writing $\mathrm{lb}(x)$ and $\mathrm{ub}(x)$ for these lower and
  upper bounds respectively, the far tail is controlled by
  $\Delta(y) := \mathrm{lb}(y) - \mathrm{ub}(y/2) \leq f(y)$, valid for
  $y \geq 1.101\times10^{26}$. These bounds are what ultimately limit
  this method's reach: since the Johnston bound requires $y \leq
  1.101\times10^{26}$, the sequence becomes uncertifiable by this
  pipeline once $Q$ approaches that limit --- which occurs at $a(26)$
...
The pipeline evaluates $G$ and $\Delta$ only at \emph{single points}: it
exits its grid loop at the first grid point $Y$ with
$\lfloor G(Y)\rfloor \geq m$, and it checks
$\Delta(1.101\times10^{26}) \geq m$ once. For those pointwise checks to
certify the entire tail, both functions must be nondecreasing beyond the
point checked. This is true, and worth stating explicitly since the
soundness of the certificate depends on it:

\begin{lemma}[Tail monotonicity]
\label{lem:tailmono}
$G$ is strictly increasing on $[5314,\, 1.101\times10^{26}]$, and
$\Delta$ is strictly increasing on $[1.101\times10^{26},\, \infty)$.
\end{lemma}

\begin{proof}
For $G$: with $L = \log y$,
\[
  G'(y) \;=\; \frac{1}{L} - \frac{1}{2\log(y/2)} - E'(y)
  \;=\; \frac{L - 2\log 2}{2L\log(y/2)} - E'(y)
  \;\geq\; \frac{L - 2\log 2}{2L^2} - \frac{L+2}{8\pi\sqrt{y}},
\]
where $E(y)$ is the error term of $G$ and the bound
$E'(y) \leq (L+2)/(8\pi\sqrt y)$ follows from
$\frac{d}{dy}\sqrt{y}\log y = (\log y + 2)/(2\sqrt y)$ applied to both
summands of $E$. Thus $G'(y) > 0$ whenever
$4\pi\sqrt{y}\,(L - 2\log 2) > L^2(L+2)$, i.e.\ whenever
$4\pi \cdot \frac{\sqrt y}{L^2}\cdot\frac{L-2\log2}{L+2} > 1$. At
$y = 5314$ the left side is $\approx 8.5$, and both factors
$\sqrt{y}/L^{2}$ (increasing for $y > e^{4}$) and $(L-2\log2)/(L+2)$
are increasing, so the inequality holds on the whole interval.

For $\Delta$: with $L = \log y$ and $\ell = \log(y/2) = L - \log 2$, using
$\frac{d}{dy}\,(y/L^k) = (L-k)/L^{k+1}$,
\[
  \Delta'(y) \;\geq\; \frac{L-1}{L^{2}}
  \;-\; \frac{1}{2\ell}\Bigl(1 + \frac{1}{\ell} + \frac{2.334}{\ell^{2}}\Bigr),
\]
where the first term keeps only the leading summand of
$\mathrm{lb}'$ (the others are positive for $L > 3$) and the second
over-estimates $\mathrm{ub}'$ term by term (using
$1 + 1/\ell + 2.334/\ell^2 \leq 1 + 2/\ell$ for $\ell \geq 3$).
Cross-multiplying, and using $L-1 \leq \ell \leq L$, positivity follows
from $2(L-1)^3 > L^2(L+2)$, which holds for all $L \geq 8$; here
$L \geq \log(1.101\times10^{26}) \approx 59.96$.
\end{proof}
...
\label{thm:sound}
Fix integers $Q \geq 5314$ and $D \geq 1$, and suppose the pipeline
terminates with output $m$, final grid point $Y \leq 1.101\times10^{26}$,
and $\Delta(1.101\times10^{26}) \geq m$. Then
\[
...
\begin{proof}
By construction $m$ is the exact minimum of $f$ on $[Q,\,Q+D]$ (the
sieved walk evaluates every change of $f$ there, including the starting
value $f(Q)$), so $\min_{y\geq Q} f(y) \leq m$ and it suffices to show
$f(y) \geq m$ for $y > Q+D$. On each grid interval $[y_i, y_{i+1}]$ the
pipeline verifies $f(y_i) - [\pcount(y_{i+1}/2) - \pcount(y_i/2)] \geq
m$, which bounds $f$ on the whole interval by
Lemma~\ref{lem:bracket}; these intervals tile $[Q+D,\,Y]$. For
$y \in [Y,\,1.101\times10^{26}]$, $f(y) \geq G(y) \geq G(Y) \geq m$ by
Lemma~\ref{lem:tailmono} and the loop's exit condition. For
$y \geq 1.101\times10^{26}$, $f(y) \geq \Delta(y) \geq
\Delta(1.101\times10^{26}) \geq m$, again by Lemma~\ref{lem:tailmono}.
Combining with Proposition~\ref{prop:identity} gives the claim.
\end{proof}
...
--- an empirical scaling exponent of roughly $0.4$--$0.7$ in $Q$, not the
...
RTX 4050 Laptop GPU, not used for this computation). Term $a(23)$ was
...
were superseded by fresh recomputation. $k=23$'s recorded time reflects
only its final continuous run segment, not the full multi-day effort
across earlier interruptions (same caveat as originally applied to
$k=21,22$; see Section~\ref{sec:performance}). Status codes are defined
below the table.}
\label{tab:results} \\
\toprule
...
23 & 949{,}634{,}047{,}203{,}617{,}038{,}366 & 26\,h\,38\,m\,31\,s\rlap{$^{\dagger}$} & 14 & F \\
\bottomrule
\end{longtable}
}
...
\textbf{F} = new result; cross-algorithm verified, independently reproduced via direct Miller--Rabin walk testing, \emph{and} its primary anchor $\pcount(Q)$ matches an independently published external reference value (Section~\ref{sec:verification}) --- the most thoroughly verified term in this table.\\
$^{\dagger}$ Not a clean measurement: this run resumed from checkpoints
cached during earlier interrupted attempts and ran concurrently with an
unrelated workload on the same machine (Section~\ref{sec:hardware}), so
26\,h\,38\,m understates the true from-scratch cost. Unlike $a(21)$ and
$a(22)$, $a(23)$ has \emph{not} been re-verified with a clean,
cache-bypassed, single-pass run; do not use this figure in any
time-complexity comparison (Section~\ref{sec:performance} already
...
$Q=10^{23}$ was certified on the desktop machine described in
Section~\ref{sec:hardware}. Its defining minimum occurs at offset $42$
above $Q$ (a dip of exactly $1$ below $f(Q)$) --- the second term in
this work, after $a(21)$, whose minimum is not simply $\pcount(Q) -
\pcount(Q/2)$. An independently published reference value for
$\pcount(10^{23})$, from the Wikipedia prime-counting-function
table~\cite{wikipedia-picount}, matches the certified primary anchor
exactly: $\pcount(10^{23}) = 1{,}925{,}320{,}391{,}606{,}803{,}968{,}923$.
The largest Ramanujan prime $\leq 10^{23}$,
$99{,}999{,}999{,}999{,}999{,}999{,}999{,}947$, was independently
confirmed prime via deterministic Miller--Rabin testing. The value
$a(23)$ itself is certified with the same rigor as every other term in
this table; only its \emph{recorded wall-clock time} is unreliable
(Table~\ref{tab:results}, note $\dagger$) and excluded from the
performance analysis in Section~\ref{sec:performance}.

\section{Verification}
\label{sec:verification}

Every new term reported here ($k=18$--$23$) passes the following
independent checks, none of which are internal self-consistency checks
against the pipeline's own prior output:

\begin{enumerate}
  \item \textbf{Cross-algorithm anchor verification.} $\pcount(Q)$ and
  $\pcount(Q/2)$ for $k=20,21,22,23$ (and, redundantly, for the
  ...
  $\pcount(10^{19})$ through $\pcount(10^{23})$; all five match this
  ...
  $f(Q)$; $a(23)$'s occurs at an offset of 42 above $Q=10^{23}$, a dip of
  exactly 1. Both were reproduced independently of \texttt{sieve128} by
  direct primality testing (via \texttt{sympy}) of every integer in the
  relevant ranges: for $a(21)$, 0 primes above and exactly 3 below; for
  $a(23)$, 0 primes above and exactly 1 below --- confirming both dips
  via entirely different machinery. $a(20)$ and $a(22)$ have their
  minimum at offset 0 (i.e.\ $a(k) = \pcount(Q)-\pcount(Q/2)$ exactly),
  so no walk verification is applicable to them.
  \item \textbf{Analytic agreement.} For $k=20,21,22,23$,
  $\li(Q)-\li(Q/2)$ agrees with the certified $f(Q)$ to within
  $10^{-10}$ relative difference in every case (respectively
  $8.2\times10^{-11}$, $2.1\times10^{-11}$, $5.3\times10^{-12}$, and
  $4.5\times10^{-12}$, computed at 60-digit working precision).
  \item \textbf{Growth-ratio consistency.} $a(k)/a(k-1)$ for
  $k=19,\dots,23$ is $9.4699$, $9.4966$, $9.5208$, $9.5427$, and
  $9.5627$ respectively --- a smooth, monotonically increasing sequence
  consistent with the expected $\sim\!9.5\times$ per-decade growth from
  prime density arguments, with no discontinuity at the transition into
  the cross-verified regime.
...
usually quoted for this class of prime-counting algorithm. The $k=23$
timing is deliberately excluded from both regressions: like the original
(superseded) $k=21,22$ measurements, it reflects only the final
continuous run segment of a computation spread across several
interrupted sessions and two concurrent workloads, so it is not a clean
single-pass data point (Table~\ref{tab:results}, caption). The number of
...
increasing with an empirical exponent of roughly $0.4$--$0.7$ in $Q$
...
A181671 to $k=20,21,22,23$, computed entirely on consumer-grade personal
hardware. The approach combines an elementary
...
This paper's prose was drafted with the assistance of an AI system
(Claude, Anthropic), directed and reviewed throughout by the author, who
verified every mathematical claim, transcribed formula, and reported
result against primary sources and independent computation
(Section~\ref{sec:verification}). The implementation ---
\texttt{ramanujan128.py}, \texttt{sieve128.cpp}, and the surrounding
build and orchestration tooling --- was likewise developed with AI
assistance (Claude, Anthropic) acting as a coding collaborator, with the
author directing the design and independently testing and verifying the
resulting code, including all cross-checks reported in this paper. The
author takes full responsibility for the correctness of all results
reported here.
